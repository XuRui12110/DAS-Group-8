...
# Prepare modelling dataset by selecting relevant variables
movies_model <- movies %>%
  select(high_rating, year, length, budget, votes, genre) %>%
  na.omit()   # Remove rows containing missing values

# Convert genre into a categorical variable (factor)
...
# --- Grouping Small/Insignificant Categories ---
# Combine Romance (n=31) and Animation (n=178) into 'Other_Genres' 
# to fix high Standard Error and extreme Odds Ratios (OR)
library(forcats)

movies_model <- movies_model %>%
  mutate(genre = fct_collapse(genre, 
    "Other_Genres" = c("Romance", "Animation")
  ))

# Set 'Action' as the reference group (largest sample size)
movies_model$genre <- relevel(movies_model$genre, ref = "Action")

# --- Model Fitting ---
# Fit the final logistic regression model
final_model <- glm(high_rating ~ year + length + budget  + genre, 
                   data = movies_model, 
                   family = binomial)

# --- Results ---
# Show model summary with P-values
summary(final_model)

# Calculate Odds Ratios (OR) and 95% Confidence Intervals
exp(cbind(OR = coef(final_model), confint(final_model)))

## So it's not necessary to merge romance and animation to the same one 
## aic is even bigger. And deviance is bigger.
```
...
The coefficient for the number of votes was not statistically significant in the full model. Therefore, a reduced model excluding this variable was fitted.

The reduced model had a slightly lower AIC (1476.6) than the full model (1478.4), indicating a slightly better balance between model fit and model simplicity.
...
newdata <- expand.grid(
  ...
  genre = c("Comedy", "Documentary", "Drama", "Animation","Action","Romance","Short") # Selected genres
)
